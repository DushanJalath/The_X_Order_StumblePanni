...
class VisaService:
    ...
    async def list_all_visas():
        ...
        return [VisaSchema(**visa) for visa in visas]

    @staticmethod
    async def get_visa_details(visa_id: str):
        return await VisaRepository.get_visa_by_id(visa_id)
    ...
    async def create_payment_intent(user_id: str, visa_id: str, amount: int, currency: str = 'usd'):
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Error creating payment intent: {str(e)}")
