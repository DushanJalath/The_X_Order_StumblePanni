...
@router.get("/visas", response_model=List[VisaApplicationResponseSchema])
async def view_visas():
    visas = await VisaService.list_all_visas()
    if not visas:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="No visa applications found")
    return visas
...
async def approve_visa(visa_id: str, officer_id: str, comments: str = ""):
    ...
    if not success:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Visa application not found or could not be approved")
...
async def deny_visa(visa_id: str, officer_id: str, comments: str = ""):
    ...
    if not success:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Visa application not found or could not be denied")
